...
def check_consecutive_absences(student):
    """
    Check if student has consecutive absences and send email alert
    Uses configurable threshold from SchoolSettings
    """
    from .models import SchoolSettings
    
    # Get configurable threshold
    settings = SchoolSettings.get_settings()
    threshold = settings.consecutive_absence_threshold
    ...
    if consecutive_absences >= threshold:
        ...
        if settings.parent_notification_enabled:
